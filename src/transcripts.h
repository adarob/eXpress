...
public:
    ...
    /**
     * a member function that returns the current (non-logged) estimated counts
    ...
    /**
     * a member function that returns the current (non-logged) variance on estimated counts
     * not valid in the first online EM round
    ...
    /**
     * a member function that increases the estimated counts and estimated coutn variance based on the probabilistic
     * assignment of a fragment
     * @param p a double for the (non-logged) probability that the fragment was generated by this transcript
     */
...
class TranscriptTable
{
    ...
    /**
     * a private bool that is true when the algorithm is being run completely online (as opposed to with additional rounds)
     */
    bool _single_round;
...
public:
     ...
     * @param globs a pointer to the struct containing pointers to the global parameter tables (bias_table, mismatch_table, fld)
     */
    TranscriptTable(const std::string& trans_fasta_file, const TransIndex& trans_index, bool single_round, double alpha, const Globals* globs);
