...
class Target {
   ...
   * at each position.
   */
  boost::scoped_ptr<std::vector<float> > _5_bias;
  /**
   * A scoped pointer to a private float vector storing the (logged) 3' bias
   * at each position.
   */
  boost::scoped_ptr<std::vector<float> > _3_bias;
  /**
...
public:
  ...
  float get_5_bias(size_t pos) const;
  float get_3_bias(size_t pos) const;
  /**
   ...
   */
  double est_effective_length(const FLD* fld, bool with_bias=true) const;
  /**
   ...
   *        for thread-safety.
   */
  void update_target_bias(BiasBoss* bias_table, FLD* fld);
  /**
...
public:
  ...
  // DOC
  std::vector<double> get_alphas(double alpha, const AlphaMap* alpha_map) const;
  /**
